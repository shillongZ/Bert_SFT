...
def load_data():
    sentences = []
    with open('Bert_SFT\stanfordSentimentTreebank\datasetSentences.txt', 'r') as f:
        next(f)
        for line in f:
    ...
    splits = []
    with open('Bert_SFT\stanfordSentimentTreebank\datasetSplit.txt', 'r') as f:
        next(f)
        for line in f:
            parts = line.strip().split(',')
            if len(parts) >= 2: 
    ...
    labels = []
    with open('Bert_SFT\stanfordSentimentTreebank\sentiment_labels.txt', 'r') as f:
        next(f)
        for line in f:
            parts = line.strip().split('|')
            if len(parts) >= 2: 
